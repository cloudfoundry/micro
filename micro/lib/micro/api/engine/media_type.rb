    ...
    module Api
      ...
      module Engine
        ...
        class MediaType
          ...
          # The subclass with the most links.
          def self.most_links
            subclasses.inject do |memo,obj|
              obj::Links.size > memo::Links.size ? obj : memo
            end
          end

          def self.graph
            GraphViz::new(:G, :type => :digraph) do |g|
              add_to_graph(g, most_links)
            end
          end

          def self.add_to_graph(graph, klass, types={})
            return  if types[klass.name]
